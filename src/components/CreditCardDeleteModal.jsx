...
const CreditCardDeleteModal = ({ isOpen, onClose, onConfirm, card, statementsCount, currentTheme }) => {
                ...
                <div className="mb-6">
                    ...
                    <div className={`${currentTheme === 'dark' ? 'bg-gray-700' : 'bg-gray-100'} rounded-lg p-3 mb-3`}>
                        <p className={`text-sm font-medium ${currentTheme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                            <strong>{t('creditCards.cardName')}:</strong> {card.name}
                        </p>
                        <p className={`text-sm ${currentTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                            <strong>{t('creditCards.bank')}:</strong> {card.bank}
                        </p>
                        {card.cardNumber && (
                            <p className={`text-sm ${currentTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                                <strong>{t('creditCards.cardNumber')}:</strong> {card.cardNumber}
                            </p>
                        )}
                    </div>
                    
                    {/* Advertencia sobre estados de cuenta */}
                    {statementsCount > 0 && (
                        <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-3">
                            <p className="text-sm text-orange-800">
                                <strong>📄 {t('creditCards.statementsWarning')}:</strong> {
                                    statementsCount === 1 
                                        ? t('creditCards.confirmDeleteWithStatement', { count: statementsCount })
                                        : t('creditCards.confirmDeleteWithStatements', { count: statementsCount })
                                }
                            </p>
                        </div>
                    )}
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                        <p className="text-sm text-yellow-800">
                            <strong>⚠️ {t('common.warning')}:</strong> {t('creditCards.deleteWarningMessage')}
                        </p>
                    </div>
                </div>
